...
ggplot(bargraph_goldDiff, aes(x = side, y = winrate, fill = iswin))+
  geom_bar(stat = "identity", position = 'dodge')+
  theme_minimal()+
  scale_fill_discrete(name = NULL,
                      labels = c("Lose", "Win"))+
  labs(title =  "Winrates for Teams with a Positive Gold Difference",
       x = "", 
       y = "Percent Winrate")

lol_ojectives_lead_towin <- high_diamond_ranked_10min %>% 
  select(ends_with(c("gameId", "blueWins", "TowersDestroyed", "Dragons", "Heralds", "EliteMonsters"))) %>% 
  mutate(objective_score_blue = rowSums(lol_ojectives_lead_towin[, c(2,4,6,8)])) %>% 
  mutate(objective_score_red = rowSums(lol_ojectives_lead_towin[, c(3,5,7,9)])) %>% 
  mutate(objscore_compare = ifelse((objective_score_blue>objective_score_red) & oneisblueWins ==1, "Expected", "Unexpected"))
