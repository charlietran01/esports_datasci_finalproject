...
barlinegraph <- HistoricalEsportData %>% 
  ...
  # group_by(era) %>% 
  group_by(year) %>% 
  mutate(sumEarnings = sum(Earnings)/1000000) %>% 
  mutate(total_annual_earnings = sum(Earnings)/1000000) 
...
ggplot(barlinegraph)+
  ...
  geom_line(aes(x = year,
             y = sumEarnings,
             color = "red"))+
  labs(title = "Professional Matches Prize Earnings",
...
# line1 <- HistoricalEsportData %>% 
#   group_by(year) %>% 
#   mutate(total_annual_earnings = sum(Earnings)/1000000) %>% 
#   ggplot(aes(x = year,
#              y = total_annual_earnings))+
#   geom_line()+
#   labs(title = "Yearly Esports Prize Earnings",
#        subtitle = "(Millions of US Dollars)",
#        x = "",
#        y = "",
#        caption = "Data from EsportsEarnings.com")+
